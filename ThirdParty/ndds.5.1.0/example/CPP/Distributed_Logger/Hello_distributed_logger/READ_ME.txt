...
Libraries would need to be loaded in the proper order. For example:
 ...
 cd "C:/rti/waveworks/ndds.5.x.y/lib/ppce500v2Vx6.9gcc4.3.3"
 ld 1 < libnddscore.so
 ld 1 < libnddsc.so
 ld 1 < libnddscpp.so
 ld 1 < librtidlc.so
...
To run this example on Windows, type the following in a command 
shell:
...
parameters. If you want to control the stack size for the initial thread,
you can use the rtpSpawn command. for instance:
