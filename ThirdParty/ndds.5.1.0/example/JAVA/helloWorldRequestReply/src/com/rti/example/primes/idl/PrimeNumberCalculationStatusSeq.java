...
/**
 * A sequence of PrimeNumberCalculationStatus instances.
 */
public final class PrimeNumberCalculationStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public PrimeNumberCalculationStatusSeq() {
        super(PrimeNumberCalculationStatus.class);
    }


    public PrimeNumberCalculationStatusSeq(int initialMaximum) {
        super(PrimeNumberCalculationStatus.class, initialMaximum);
    }


    public PrimeNumberCalculationStatusSeq(Collection elements) {
        super(PrimeNumberCalculationStatus.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, PrimeNumberCalculationStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' PrimeNumberCalculationStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PrimeNumberCalculationStatus.create());
