...
/**
 * A sequence of PrimeNumberRequest instances.
 */
public final class PrimeNumberRequestSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public PrimeNumberRequestSeq() {
        super(PrimeNumberRequest.class);
    }


    public PrimeNumberRequestSeq(int initialMaximum) {
        super(PrimeNumberRequest.class, initialMaximum);
    }


    public PrimeNumberRequestSeq(Collection elements) {
        super(PrimeNumberRequest.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, PrimeNumberRequest.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' PrimeNumberRequest objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PrimeNumberRequest.create());
