...
$JAMAICAC -useTarget $TARGETPLATFORM -d objs -classpath "$NDDSHOME/class/nddsjava.jar":"$NDDSHOME/class/distlog.jar" $ALL_SRC

echo "Generating code for Jamaica target $TARGETPLATFORM and RTI target $RTI_EXAMPLE_ARCH ..."
$JAMAICABUILDER -XdefineProperty+=java.util.logging.config.file=logging.properties -classpath objs:"$NDDSHOME"/class/nddsjava.jar:"$NDDSHOME"/class/distlog.jar -includeJar="$NDDSHOME"/class/nddsjava.jar:"$NDDSHOME"/class/distlog.jar -XlibraryPaths="$NDDSHOME"/lib/${RTI_EXAMPLE_ARCH} -XstaticLibraries="nddsjavaz" -setGraphics=none -interpret -target=$TARGETPLATFORM -destination=com.rti.hello.Hello com.rti.hello.Hello
