...
#    mkdir -p com/rti/hello/idl
    $NDDSHOME/scripts/rtiddsgen -package com.rti.hello.idl -language Java -replace HelloWorld.idl
    cd ..
fi
...
$JAMAICAC -useTarget $TARGETPLATFORM -d objs -classpath "$NDDSHOME/class/nddsjava.jar":"$NDDSHOME/class/distlog.jar":"$NDDSHOME/class/log4j-1.2.16.jar" $ALL_SRC

echo "Generating code for Jamaica target $TARGETPLATFORM and RTI target $RTI_EXAMPLE_ARCH ..."
$JAMAICABUILDER -classpath objs:"$NDDSHOME"/class/nddsjava.jar:"$NDDSHOME"/class/distlog.jar:"$NDDSHOME"/class/log4j-1.2.16.jar -includeJar="$NDDSHOME"/class/nddsjava.jar:"$NDDSHOME"/class/distlog.jar:"$NDDSHOME"/class/log4j-1.2.16.jar -XlibraryPaths="$NDDSHOME"/lib/${RTI_EXAMPLE_ARCH} -XstaticLibraries="nddsjavaz" -setGraphics=none -interpret -target=$TARGETPLATFORM -destination=com.rti.hello.Hello com.rti.hello.Hello
