...
<!-- 
/* $Id: typeSubscriber.cpp.cli.xsl,v 1.2 2012/04/23 16:44:18 fernando Exp $
...
10v,17mar08,rbw Removed explicit disposal from example
    ...
    <xsl:if test="$lastTopLevel = 'yes'">
        <xsl:call-template name="replace-string-from-map">
            ...
            <xsl:with-param name="inputString">
                <xsl:text><![CDATA[/* %%IDLFileBaseName%%_subscriber.cpp
...
public ref class %%nativeType%%Subscriber {
public:
...
public ref class %%nativeType%%Listener : public DDS::DataReaderListener {
  public:
    ...
    %%nativeType%%Listener() {
        data_seq = gcnew %%nativeTypeC%%Seq();
        info_seq = gcnew DDS::SampleInfoSeq();
    }

  private:
    %%nativeTypeC%%Seq^ data_seq;
...
void %%nativeType%%Subscriber::subscribe(
...
/* Delete all entities */
void %%nativeType%%Subscriber::shutdown(
        DDS::DomainParticipant^ participant) {
...
void %%nativeType%%Listener::on_data_available(DDS::DataReader^ reader) {
    %%nativeTypeC%%DataReader^ %%nativeType%%_reader =
        safe_cast<%%nativeTypeC%%DataReader^>(reader);
    ...
    for (int i = 0; i < data_length; ++i) {
        if (info_seq->get_at(i)->valid_data) {
            %%nativeTypeC%%TypeSupport::print_data(data_seq->get_at(i));
        }
    }
